...
import static com.mayateam.client.ClientUtil.openSocket;
import static com.mayateam.client.ClientUtil.writeMessage;
...
public class ClientUtilTest {
    ...
    void openSocketCreatesSocketConnectedToServer(int port) {
        try (ServerSocket serverSocket = new ServerSocket(port);
             Socket clientSocket = openSocket(serverSocket.getInetAddress().getHostAddress(), port)) {
    ...
    void writeToSocketSendsMessageViaOutputStream(String givenMessage) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(port)) {
            ...
            try (Socket clientSocket = new Socket(InetAddress.getLocalHost().getHostAddress(), port)) {

                writeMessage(givenMessage, clientSocket);
